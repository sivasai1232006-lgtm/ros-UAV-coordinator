...
class UAV2Node(Node): # MODIFY node name
    ...
    def __init__(self):
        ...
        self.state_ = "IDLE"
        self.status_pub_ = self.create_publisher(UavStatus, "/uav2/status", 10)
        self.chech_battery_server_ = self.create_service(CheckBattery, 'uav2/check_battery', self.check_battery_callback)
...
def main(args=None):
    ...
    #My node
    node = UAV2Node() # MODIFY node name
    rclpy.spin(node)
    rclpy.shutdown()
